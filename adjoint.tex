...
Let $\Gamma$ be a congruence subgroup--in our examples, we will take $\Gamma_0(N)$. Our primary objects of study will be the compactly supported cohomology groups $H_c^*(\Gamma,M)$, which is isomorphic to the compactly supported cohomology of the open modular curve $\cH/\Gamma$. These cohomology groups can be concretely described in terms of Modular Symbols, which we briefly recall.

Denote by $\Delta_0$ the degree-$0$ divisors on $\mathbb{P}^1(\QQ)$, equipped with the left action of $\Gamma$ by fractional-linear-transformations. Let $M$ be any right $\ZZ[\Gamma]$-module. The space of $M$-valued modular symbols for $\Gamma$ is defined as
\begin{equation}
