...
The pairing $M \times M \to E$ from Section \ref{sec:coeff_pairing} then induces a natural cup product pairing $\Hh^1_P(\Gamma_0(N), M) \times \Hh^1_P(\Gamma_0(N), M) \to \Hh^2_P(\Gamma_0(N), E)$.  In order to compute this pairing, we need to give an explicit isomorphism $\Hh^2_P(\Gamma_0(N), E) \to E$, which we do in two steps.

First, the corestriction map $\cor : \Hh^2(\Gamma_0(N), E) \to \Hh^2(\SL_2(\ZZ), E)$ is given by multiplication by $[\SL_2(\ZZ) : \Gamma_0(N)]$.  
